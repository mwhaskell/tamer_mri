function [ fit_hf, x, pcg_out] = mt_fit_fcn_v8p_over( dM_in, dM_in_indices, Mn, Cfull, km, ...
    tse_traj, U , tar_pxls , full_msk_pxls, xprev, exp_str, kfilter, Uset)
...
Afxf = A(xs_v_f,U,Cfull,tse_traj,Ms,fixed_pxls,np,Uset{1}) + ...
    A(xs_v_f,U,Cfull,tse_traj,zeros(size(Ms)),fixed_pxls,np,Uset{2});


AtsAfxf = Astar(Afxf,U,Cfull,tse_traj,Ms,tar_pxls,Uset{1}) + ...
    Astar(Afxf,U,Cfull,tse_traj,zeros(size(Ms)),tar_pxls,Uset{2});

RHS = Astar(km,U,Cfull,tse_traj,Ms,tar_pxls,Uset{1}) + ...
    Astar(km,U,Cfull,tse_traj,zeros(size(Ms)),tar_pxls,Uset{2}) - AtsAfxf;

if (~isempty(xprev))
[xs_v_t, f, rr, it] = pcg(@(x)...
    LHS(x,U,Cfull,tse_traj,Ms,lambda,tar_pxls,np,Uset), RHS, 1e-3, iters, [], [],...
...
%% project back
ks = A(xs_v,U,Cfull,tse_traj,Ms,full_msk_pxls,np,Uset{1}) + ...
    A(xs_v,U,Cfull,tse_traj,zeros(size(Ms)),full_msk_pxls,np,Uset{2});
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%              A                       %%%%%%%%%%%%%%%%%%%%%%%%%%%
function [kdata] = A(x,U,Cfull,tse_traj,Ms,nz_pxls_in,np,Uover)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%              Astar                   %%%%%%%%%%%%%%%%%%%%%%%%%%%
function [imdata] = Astar(k,U,Cfull,tse_traj,Ms,nz_pxls_in,Uover)
...
%%           LHS function                                        %%%%%%%%%%
function [output] = LHS(x,U,Cfull,tse_traj,Ms,lambda,nz_pxls,np,Uset)

Ax = A(x,U,Cfull,tse_traj,Ms,nz_pxls,np,Uset{1}) + ...
    A(x,U,Cfull,tse_traj,zeros(size(Ms)),nz_pxls,np,Uset{2});

AsAx = Astar(Ax,U,Cfull,tse_traj,Ms,nz_pxls,Uset{1}) + ...
    Astar(Ax,U,Cfull,tse_traj,zeros(size(Ms)),nz_pxls,Uset{2});
