function [ fit_hf, x, pcg_out, ks] = tamer_obj_func_par( dM_in, dM_in_indices, Mn, Cfull, km, ...
    tse_traj, U , tar_pxls , full_msk_pxls, xprev, exp_str, kfilter, rI_off)
...
Afxf = A(xs_v_f,U,Cfull,tse_traj,Ms,fixed_pxls,rI_off);
AtsAfxf = Astar(Afxf,U,Cfull,tse_traj,Ms,tar_pxls,rI_off);
RHS = Astar(km,U,Cfull,tse_traj,Ms,tar_pxls,rI_off) - AtsAfxf;

if (~isempty(xprev))
[xs_v_t, f, rr, it] = pcg(@(x)...
    LHS(x,U,Cfull,tse_traj,Ms,lambda,tar_pxls,rI_off), RHS, 1e-3, iters, [], [],...
...
%% project back
ks = A(xs_v,U,Cfull,tse_traj,Ms,full_msk_pxls,rI_off);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%              Astar                   %%%%%%%%%%%%%%%%%%%%%%%%%%%
function [imdata] = Astar(k,U,Cfull,tse_traj,Ms,nz_pxls_in, rI_off)
...
%%           LHS function                                        %%%%%%%%%%
function [output] = LHS(x,U,Cfull,tse_traj,Ms,lambda,nz_pxls, rI_off)

Ax = A(x,U,Cfull,tse_traj,Ms,nz_pxls,rI_off);
AsAx = Astar(Ax,U,Cfull,tse_traj,Ms,nz_pxls,rI_off);
output = AsAx + lambda;
