function [ fit, x, pcg_out, ktheta] = tamer_obj_func_par( dTheta_in,...
    mt2corr, theta_prev, sens, kdata, tse_traj, U , recon_vxls , ...
    msk_vxls, xprev, kfilt, rI_off)
%
% TAMER objective function (Eqn 6 in paper) calculated in parallel
%
% INPUTS:
%
...
Ffxf = E(xs_v_f,U,sens,tse_traj,theta,fixed_vxls,rI_off);
EHfEfxf = EH(Ffxf,U,sens,tse_traj,theta,recon_vxls,rI_off);
RHS = EH(kdata,U,sens,tse_traj,theta,recon_vxls,rI_off) - EHfEfxf; %RHS-"right hand side"

if (~isempty(xprev))
[xs_v_t, f, rr, it] = pcg(@(x)...
    EHE(x,U,sens,tse_traj,theta,lambda,recon_vxls,rI_off), RHS, 1e-3, iters, [], [],...
...
pxl_per_sli = numel(msk_vxls)/nsli;
xs_v = zeros(numel(msk_vxls),1);
...
%% project back
ktheta = E(xs_v,U,sens,tse_traj,theta,msk_vxls,rI_off);
...
function [imdata] = EH(k,U,sens,tse_traj,theta,recon_vxls, rI_off)
...
parfor t = 1:tls
    ...
    temp_vol2 = rot3d(temp_vol2,yaw,pitch,roll, rI_off);
    ...
    inv_rot_vol = rot3d(temp_vol,-1*yaw,-1*pitch,-1*roll, rI_off);
...
function [output] = EHE(x,U,sens,tse_traj,theta,lambda,recon_vxls, rI_off)

Ex = E(x,U,sens,tse_traj,theta,recon_vxls,rI_off);
EHEx = EH(Ex,U,sens,tse_traj,theta,recon_vxls,rI_off);
output = EHEx + lambda;
